...
def crop_face_only(image):
    ...
    if coords is None:
        return image  # fallback if no non-zero pixels
...
def frontalize(img, proj_matrix, ref_U, eyemask):
    ...
    face_mask = np.sum(np.abs(ref_U), axis=2) > 0
    face_mask = face_mask.astype(np.uint8)
    face_mask = np.repeat(face_mask[:, :, np.newaxis], 3, axis=2)

    frontal_raw_masked = frontal_raw * face_mask
    frontal_sym_masked = frontal_sym * face_mask

    cropped_frontal_raw = crop_face_only(frontal_raw_masked)
    cropped_frontal_sym = crop_face_only(frontal_sym_masked)

    return frontal_raw, frontal_sym, frontal_raw_masked, frontal_sym_masked, cropped_frontal_raw, cropped_frontal_sym
